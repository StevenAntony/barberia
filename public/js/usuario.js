...
btnEnviarForm.click(function () {  
    $.ajax({
        ...
        dataType: "json",
        url: accion == 'Nuevo' ? idFormEnviar.attr('action') : `/usuario/update/${elementEditar._id}`,
...
$(document).on('click','.editarInfo', function () {
    ...
    $('input[name="itmEmail"]').val(elementEditar.Email)
    $('input[name="itmRol"]').val(elementEditar.Rol)
    $('input[name="itmUsuario"]').val(elementEditar.Usuario)
    accion = 'Editar'
...
$(document).on('click','.cambiarEstado',function () {  
    ...
    $.ajax({
        type: 'put',
        dataType: "json",
        url: `/usuario/estado/${table.row(tr).data()._id}`,
...
const formatRow = (row) => {
    return (`<table class="table" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">
                ...
                <tr><td>Usuario</td><td>${row.Usuario}</td></tr>
                <tr><td>Correo</td><td>${row.Email}</td></tr>
                <tr><td>Rol</td><td>${row.Rol}</td></tr>
...
$(document).ready(function () {
    table = $("#dataTableInfo").DataTable({
        ...
        columnDefs: [
            ...
            {title: "Usuario",targets: [ 2 ],visible: true},
            {title: "Correo",targets: [ 3 ],visible: true},
            {title: "Rol",targets: [ 4 ],visible: true},
        ...
        columns: [
            ...
            {className:'text-center hide-xs',orderable:false,data:'Usuario',defaultContent: ''},
            {className:'text-center',orderable:false,data:'Email',defaultContent: ''},
            {className:'text-center hide-xs',orderable:false,data:'Rol',defaultContent: '',
                render : function (data,type,row) {  
                    return `<span class='badge ${row.Rol == 'Admin' ? 'bg-info':'bg-secondary'} '>${row.Rol}</span>`
                }
            },
